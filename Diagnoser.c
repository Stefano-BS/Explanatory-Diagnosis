...
void regexMake(Regex* s1, Regex* s2, Regex* d, char op, Regex *s3) {
    ...
    if (op == 'c') {                                                        // Concatenation
        if (strl1>0 && strl2>0) { // concat
            ...
            else if (streq12 && (s1->regex[strl1-1]=='*' || s1->regex[strl1-1]=='+')) {
                solLen = strl1;
                solution = s1->regex;
        ...
        if (strl1 > 0 && streq12) { // Copia s1 in d
            ...
            solution = regexBuf;
            d->concrete = s1->concrete && s2->concrete;
            if (strl1==strl2+1 && (s1->regex[strl2]=='?' || s1->regex[strl2]=='+' || s1->regex[strl2]=='*') && strncmp(s1->regex, s2->regex, strl2)==0) {
                solution = s1->regex;
                solLen = strl1;
                d->bracketed = true;
            }
            else if (strl2==strl1+1 && (s2->regex[strl1]=='?' || s2->regex[strl1]=='+' || s2->regex[strl1]=='*') && strncmp(s1->regex, s2->regex, strl1)==0) {
                solution = s2->regex;
                solLen = strl2;
                d->bracketed = true;
            }
